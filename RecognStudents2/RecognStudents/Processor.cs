...
namespace AForge.WindowsForms
{
    internal class Settings
    {
		...
		public BaseNetwork Network { get; set; }
	...
	internal class MagicEye
    {
        ...
        public bool ProcessImage(Bitmap bitmap)
        {
            ...
            if (settings.processImg)
            {
				...
				Sample sample = DataLoader.ConvertToSample(uProcessed.ToManagedImage(),10, (FigureType)10);

				// 4. Пропустить через нейросеть
				if (settings.Network != null)
				{
					settings.Network.Predict(sample);
					info = $"Класс: {sample.recognizedClass}";
				}
        ...
        private string processSample(ref Imaging.UnmanagedImage unmanaged)
        {
			...
			int padding = 10; // или вычисляем как процент от размера blob'а
